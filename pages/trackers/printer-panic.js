...
export async function getServerSideProps() {
  ...
  try {
    const res = await fetch('https://api.stlouisfed.org/fred/series/observations?series_id=RRPONTSYD&api_key=c820e0ce56eee34bb625b87367825a49&file_type=json&limit=2');
    const data = await res.json();

    const latest = data.observations[data.observations.length - 1];
    const prev = data.observations[data.observations.length - 2];

    const rrp = parseFloat(latest.value);
    const trend = rrp > parseFloat(prev.value) ? '↑' : rrp < parseFloat(prev.value) ? '↓' : '→';
    let status = 'SAFE';
    if (rrp < 100000) status = 'RISK';
    if (rrp < 60000) status = 'PANIC';
